...
const EventPL = () => {
  ...
  const { myReportseventData, loading } = useSelector((state) => state.downline);
  ...
  useEffect(() => {
    dispatch(getMyRepostsDataByEvents({
      page,
      limit,
      startDate,
      endDate,
                ...
                <tbody>
                    ...
                    PLdata.map((item, index) => (
                      ...
                      >
                        <td
                          ...
                          onClick={() => navigate(`/eventplteams/${item.name}`)}
                        >
                          {item.name}
                        </td>
                        <td
                          className={`border border-gray-300 p-2 ${item.myProfit > 0
                            ? "text-green-500"
                            : "text-red-500"
                            }`}
                        >
                  ...
                  {!loading && PLdata?.length > 0 && (
                    <tr className="text-center font-semibold" >
                      ...
                      <td className={`border border-gray-300 p-2 ${Totaldata.myProfit > 0
                        ? "text-green-500"
                        : "text-red-500"
                        }`}>{Math.abs(Totaldata.myProfit)}</td>
                      <td className={`border border-gray-300 p-2 ${Totaldata.myProfit > 0
                        ? "text-red-500"
                        : "text-green-500"
                        }`}>  {Math.abs(Totaldata.myProfit)}</td>
