...
import GamePlayModal from "../components/GamePlayer";
        ...
        <div className="max-w-[1400px] mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
            {currentGames.map((game, index) => (
              <div
                key={`${game.id || game.game_uid}-${index}`}
                onClick={() => handlePlayGame(game)}
                className="group cursor-pointer bg-white rounded overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-all"
              >
                <div className="relative aspect-[1/1.2] overflow-hidden bg-gray-100">
                  <img
                    src={game.img || game.icon}
                  ...
                  <div className="absolute inset-0 bg-white/20 opacity-0 group-hover:opacity-100 flex items-center justify-center">
                    {launchLoading &&
                    selectedGame?.game_uid === game.game_uid ? (
                      <FaSpinner className="animate-spin text-xl text-red-600" />
                    ) : (
                      <FaPlay className="text-xl text-red-600" />
                    )}
                  </div>
                </div>
