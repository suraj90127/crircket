...
const SoccerOver5 = ({ matcUnder5List, gameid, match }) => {
    ...
    const matcUnder5 =
        Array.isArray(matcUnder5List) &&
            ...
            ? matcUnder5List[0].section.map((sec) => ({
                ...
                sid: sec.sid,
                odds: sec.odds,
                mname: matcUnder5List[0].mname, // ✅ Access from first item
                status: matcUnder5List[0].status, // ✅ Access from first item
            }))
            : [];
    ...
    function MyComponent({ team, matchData, pendingBet, myAmount, index, selectedRun }) {
        ...
        const isSelected =
            selectedRun?.type === "OVER_UNDER_05" &&
                            ...
                            <div className="mx-auto text-[13px] bg-gray-200">
                                ...
                                {matcUnder5[0]?.status === "SUSPENDED" ? (
                                    <div className="border-2 border-red-500 relative mx-auto">
                                        ...
                                        {matcUnder5.map(({ team, odds, sid }, index) => (
                                            <div key={index}>
                                                <div
                                    ...
                                    <div>
                                        <div className="grid grid-cols-9 text-center border-b border-gray-300 bg-white">
                                            ...
                                            <div className="font-semibold p-1 text-[11px] rounded-lg col-span-2 hidden md:block">
                                                <div className="bg-[#bed5d8] rounded-md p-0.5">
                                                    <span className="text-[#315195]">Min/Max </span>{matcUnder5List[0]?.min}-{matcUnder5List[0]?.maxb}
                                                </div>
                                            </div>
                                        </div>
                                        {matcUnder5.map(({ team, odds, sid }, index) => (
                                                    ...
                                                    {odds.map((odd, i) => (
                                                        ...
                                                        >
                                                            <div
                                                                onClick={() => {
                                                                    if (odd?.odds !== 0) {
                                                                        handleSelect("OVER_UNDER_05", index, odd?.odds, odd?.otype);
                                                                        setValue(odd?.odds, team, odd.otype);
                                                                    }
                                                                }}
                                                            >
